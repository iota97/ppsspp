...
FPURegCache::FPURegCache() : mipsState(0), initialReady(false), emit(0) {
...
X64Reg FPURegCache::LoadRegsVS(const u8 *v, int n) {
	...
	if (sequential == n && regsLoaded < n) {
		...
		const float *f = v[0] < 128 ? &mipsState->v[voffset[v[0]]] : &mipsState->tempValues[v[0] - 128];
...
void FPURegCache::StoreFromRegister(int i) {
	...
	if (regs[i].away) {
		...
		if (regs[i].lane != 0) {
			...
			const float *f = mri[0] - 32 < 128 ? &mipsState->v[voffset[mri[0] - 32]] : &mipsState->tempValues[mri[0] - 32 - 128];
	...
	for (int i = 0; i < NUM_MIPS_FPRS; i++) {
		...
		if (regs[i].away) {
			if (regs[i].location.IsSimpleReg()) {
				...
				_assert_msg_(false, "Jit64 - Flush unhandled case, reg %i PC: %08x", i, mipsState->pc);
			}
		}
	}
	pendingFlush = false;
...
OpArg FPURegCache::GetDefaultLocation(int reg) const {
	if (reg < 32) {
		...
		if (useRip_) {
			return M(&mipsState->v[voffset[reg - 32]]);  // rip accessible
		} else {
			return MIPSSTATE_VAR_ELEM32(v[0], voffset[reg - 32]);
		}
	} else {
		if (useRip_) {
			return M(&mipsState->tempValues[reg - 32 - 128]);  // rip accessible
		} else {
